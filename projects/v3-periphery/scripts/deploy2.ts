...
async function main() {
  ...
  const swapRouter = await SwapRouter.connect(owner).deploy(pancakeV3PoolDeployer_address, pancakeV3Factory_address, config.WNATIVE)
  ...
  await nonfungibleTokenPositionDescriptor.connect(owner).deployed()
  console.log('nonfungibleTokenPositionDescriptor', nonfungibleTokenPositionDescriptor.address)
  ...
  const nonfungiblePositionManager = await NonfungiblePositionManager.connect(owner).deploy(
  ...
  const pancakeInterfaceMulticall = await PancakeInterfaceMulticall.connect(owner).deploy()
  ...
  const v3Migrator = await V3Migrator.connect(owner).deploy(
    pancakeV3PoolDeployer_address,
    pancakeV3Factory_address,
    config.WNATIVE,
  ...
  const tickLens = await TickLens.connect(owner).deploy()
  ...
  const quoterV2 = await QuoterV2.connect(owner).deploy(pancakeV3PoolDeployer_address, pancakeV3Factory_address, config.WNATIVE)
