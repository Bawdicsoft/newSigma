...
const SigmaTokenDeployed = "0xE791872720e48A7EF94aF59e11266A4C12a21D7A"
const USDTdeployed = "0x6B1f36A9f5b2F8690a4968a27beD5a94c5D874ad"
const SigmaV3Deployed = "0x24d2a152d84D831601f05F7DDDc4ed9EC18295BC"
...
const sigmaV3ABI = [
  ...
  const usdt = new ethers.Contract(USDTdeployed, usdtABI, provider); 
  const sigmaTokenContract = new ethers.Contract(SigmaTokenDeployed, sigmaTokenABI, provider); 
  const sigmaV3Contrat = new ethers.Contract(SigmaV3Deployed, sigmaV3ABI, provider); 
...
async function transferSigma(){
  ...
  for (let i = 0; i < 31; i++) {
    ...
    const owner = new ethers.Wallet(privateKey, provider);
    const amount = ethers.parseEther("100")
    const tx = await sigmaTokenContract.connect(owner).transfer(wallet, amount);
    await tx.wait();
    console.log(tx.hash);
...
async function balanceTester(){
  ...
  let a = async (num) => {
    ...
    const wallet = new ethers.Wallet(ad1);
    // Get the signer from the wallet
    signer = await wallet.connect(provider);
  ...
  for (let i = 0; i < 31; i++) {
    // let wallet = (await a(i)).toString();
    const addressFromA = await a(i);
    console.log(`Return value from a(${i}):`, addressFromA);
    let wallet = addressFromA;
    console.log(`${i} : ${wallet.address }`);
    try {
      // const balance = await sigmaTokenContract.connect(wallet).balanceOf(wallet.address);
      const tx = await usdt.connect(wallet).balanceOf(wallet.address);
      await tx.wait();
      // const balanceUSDT = await usdt.balanceOf(wallet.address);
      console.log("Balance:", Number(tx));
  } catch (error) {
      console.error("Error getting balance:", error);
  }
    // const tx = await sigmaTokenContract.balanceOf(wallet);
    // await tx.wait();
    // // const tx2 = await usdt.balanceOf(wallet);
    // // await tx2.wait();
    // console.log(tx);
...
async function EthBalance(){
  ...
  for (let i = 0; i < 31; i++) {
    ...
    console.log(`Balance of ${wallet.address}: ${balanceEth} ETH`);
...
async function transferSigma2(){
  ...
  for (let i = 0; i < 2; i++) {
    ...
    // await tx.wait();
    // console.log(tx);
    const balance = await sigmaTokenContract.balanceOf(wallet); // Call balanceOf directly
