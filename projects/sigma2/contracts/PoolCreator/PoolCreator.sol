...
contract PoolCreator{
    ...
    IPancakeV3Factory pancakeV3Factory;
    address public factoryAddress;
    // Mappings 
    // Constructor 
    constructor(address _factoryAddress) {
        factoryAddress = _factoryAddress;
        pancakeV3Factory = IPancakeV3Factory(_factoryAddress);
    }
   
    // Write Functions 
    function CreatePool(address tokenA, address tokenB, uint24 _fee) external returns(address){
        TransferHelper.safeApprove(tokenA, factoryAddress, type(uint256).max);
        TransferHelper.safeApprove(tokenB, factoryAddress, type(uint256).max);
        _pool = pancakeV3Factory.createPool(tokenA, tokenB, _fee);
        return _pool;
    }
    // read Functions
