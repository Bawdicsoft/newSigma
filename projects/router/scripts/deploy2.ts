...
async function main() {
  ...
  const smartRouterHelper = await SmartRouterHelper.connect(owner).deploy()
  ...
  const smartRouter = await SmartRouter.connect(owner).deploy(
    config.v2Factory,
  ...
  const mixedRouteQuoterV1 = await MixedRouteQuoterV1.connect(owner).deploy(
    pancakeV3PoolDeployer_address,
    pancakeV3Factory_address,
    config.v2Factory,
    config.stableFactory,
  ...
  const tokenValidator = await TokenValidator.connect(owner).deploy(config.v2Factory, positionManager_address)
